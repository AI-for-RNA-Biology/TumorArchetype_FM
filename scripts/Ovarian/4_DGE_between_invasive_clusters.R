...
seurat_object_labeled <- seurat_object_labeled[ , rownames(seurat_object_labeled@meta.data)[seurat_object_labeled@meta.data$label != "nan"]]
...
# Load config file
config <- fromJSON(here("config", "config_notebooks_Ovarian.json"))
...
cat("DEBUG: Number of matches =", length(path_labels), "\n")
cat("DEBUG: Matched files:\n")
print(path_labels)
...
# res_true_labels <- load_dge_pathways_analysis_per_clusters(directory_name = here("results", "Ovarian", "molecular", "tissue_DGE"), add_name = "_upregulated_true_labels_all")
res_true_labels <- load_dge_pathways_analysis_per_clusters(directory_name = here("results", "Ovarian", "molecular", "tissue_DGE"), add_name = "_upregulated_true_labels_all")

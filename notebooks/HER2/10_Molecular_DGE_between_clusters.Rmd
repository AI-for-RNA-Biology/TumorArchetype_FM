...
jupyter:
  kernelspec:
    name: it_digitalhisto
    display_name: "R (digitalhisto)"
---
...
rds_file <- here("results", "molecular", "gene_embeddings_HER2.rds")
...
if (file.exists(rds_file)) { 
  ...
  # Read the .csv file
    csv_file <- here("results", "molecular", "gene_embedding_HER2.csv")
    m <- read.csv(csv_file, row.names="index")
    # Read the metadata
    spots_metadata_file <- here("results", "molecular", "spots_metadata.csv") 
  ...
  print(cell_counts_per_patient)
    ...
    saveRDS(seurat_object, file = here("results", "molecular", "gene_embeddings_HER2.rds"))
  
    cat("Seurat object created and saved to", here("results", "molecular", "gene_embeddings_HER2.rds"), "\n") 
    }
...
write.csv(as.matrix(rna_data), file = here("results", "molecular", "filtered_normalized_gene_expression.csv"))
```
...
# Define here the model you want to check
# models <- c("uni", "uni_full_koleo_16384_prototypes")
models <- c("uni", "uni_full_koleo_16384_prototypes", "uni_full_koleo_32768_prototypes", "uni_full_kde_32768_prototypes")
# models <- c("uni_full_kde_4096_prototypes",
...
config <- fromJSON(here("config", "config_notebooks.json"))

benchmark_folders <- c()

for (m in models)  {
  if (m %in% config$retrained_model_list1) {
    relative_path <- sub("^\\.\\./", "", config$retrained_benchmark_folder1)
    benchmark_folder <- here(relative_path)
  } else {
    relative_path <- sub("^\\.\\./", "", config$retrained_benchmark_folder2)
    benchmark_folder = here(relative_path)
  }
  benchmark_folders <- c(benchmark_folders, benchmark_folder)
}

files <- c()
for (i in seq_along(models)) {
  model <- models[i]
  folder <- benchmark_folders[i]
  ...
  path_label <- path_labels[1]
  
  files <- c(files, path_label)
}
...
for (file in files) {
  ...
  results_upregulated[[file]] <- get_pathways_heatmaps(labels_clusters_uni_file = file, seurat_object = seurat_object, res_true_labels = res_true_labels, upregulated = TRUE, add_name = paste0("_", n, "_clusters"))
  results_downregulated[[file]] <- get_pathways_heatmaps(labels_clusters_uni_file = file, seurat_object = seurat_object, res_true_labels = res_true_labels_down, upregulated = FALSE, add_name = paste0("_", n, "_clusters"))
}
```

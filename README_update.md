...
- `--dataset`: the dataset to process. Choose among "HER2" and "TNBC". Default is "HER2".
...
- `--model_name`: the pretrained model to be used. The supported values are "uni", "ctranspath", "provgigapath", "simclr", "virchow". You can add other models in `digitalhistopathology/models.py`. Default is "uni".
...
In the context of our study, we were interested in the invasive cancer patches, and after the kNN-relabeling of step 2, we created a filtered dataset to keep only the patches relabeled as "invasive cancer" from the pipeline applied to UNI embeddings.
